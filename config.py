...
FACE_CLUSTER_MERGE_STRONG_LINK_SIM = float(os.getenv("FACE_CLUSTER_MERGE_STRONG_LINK_SIM", "0.82")) # 单对代表脸极高相似时的碎簇合并阈值
FACE_CLUSTER_MERGE_STRONG_MIN_CENTROID_SIM = float(os.getenv("FACE_CLUSTER_MERGE_STRONG_MIN_CENTROID_SIM", "0.50")) # 强桥接合并时要求的最低簇中心相似度
...
FACE_MIN_CROP_RATIO = float(os.getenv("FACE_MIN_CROP_RATIO", "0.08"))   # gallery 裁剪图短边 / 关键帧短边 的比例下限
FACE_MIN_CROP_PIXEL_FLOOR = float(os.getenv("FACE_MIN_CROP_PIXEL_FLOOR", "48")) # gallery 裁剪图短边绝对像素兜底下限
FACE_REJECT_SIDE_FACE = os.getenv("FACE_REJECT_SIDE_FACE", "true").lower() in ("1", "true", "yes", "on") # 是否过滤侧脸
FACE_MAX_POSE_YAW = float(os.getenv("FACE_MAX_POSE_YAW", "35"))         # InsightFace pose yaw 绝对值超过该角度视为侧脸
FACE_MAX_LANDMARK_IMBALANCE = float(os.getenv("FACE_MAX_LANDMARK_IMBALANCE", "0.35")) # 5点 landmarks 鼻尖偏离双眼中心的最大相对比例
