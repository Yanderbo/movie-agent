...
def run_minute_chunk_understand(
    ...
    for s in shots:
        if s.scene_index not in covered_indices:
            ...
            shot_trans = [t for t in all_transcripts if t.scene_index == s.scene_index]
            speaking = sorted({t.character_id for t in shot_trans if t.character_id})
            all_alignments.append(MultimodalAlignment(
                scene_index=s.scene_index,
                start_time=s.start_time,
                end_time=s.end_time,
                visible_characters=[],
                speaking_characters=speaking,
                active_modalities=["speech"] if shot_trans else [],
                dominant_modality="speech" if shot_trans else "visual",
                alignment_confidence=0.0,
                notes="模型未返回该镜头 per_shot，写入占位对齐",
            ))
...
def _resolve_per_shot_scene_index(item, shot_map, local_to_global, scene_index_mode) -> int | None:
    """解析 per_shot 对应的全局 scene_index，并处理双索引冲突。"""
    local_idx = _coerce_int(item.get("local_shot_index"))
    local_scene = local_to_global.get(local_idx) if local_idx in local_to_global else None

    scene_idx = _coerce_int(item.get("scene_index"))
    global_scene = scene_idx if scene_idx in shot_map else None
    scene_as_local = local_to_global.get(scene_idx) if scene_idx in local_to_global else None

    if local_scene is not None and global_scene is not None:
        if local_scene != global_scene:
            if scene_index_mode == "local":
                logger.warning(
                    "  per_shot 索引冲突: "
                    f"local_shot_index={local_idx} -> scene_index={local_scene}, "
                    f"返回 scene_index={scene_idx} 同时命中全局 shot；"
                    "整体判断为局部索引，优先使用 local_shot_index"
                )
                return local_scene
            logger.warning(
                "  per_shot 索引冲突: "
                f"local_shot_index={local_idx} -> scene_index={local_scene}, "
                f"但返回 scene_index={scene_idx}；优先使用全局 scene_index"
            )
            return global_scene
        return local_scene

    if global_scene is not None:
        if scene_index_mode == "local" and scene_as_local is not None:
            return scene_as_local
        return global_scene
...
def _update_profiles(profiles, result, chunk_index):
    ...
    for update in updates:
        ...
        if cid in profiles:
            ...
            if change_desc and not _is_placeholder_text(change_desc):
                # 回写 description，但过滤“无/无法判断”等占位描述。
                p.description = change_desc

            key_action = _safe_text(update.get("key_action", ""))
            if key_action and not _is_placeholder_text(key_action):
            ...
            if change_desc:
                profile.appearance_changes.append({
                    "chunk_idx": chunk_index,
                    "description": change_desc,
                })
